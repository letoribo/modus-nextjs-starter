...
import { GREETING, GET_QUOTE } from "@/lib/queries"
import styles from '@/styles/dashboard.module.css'

const Dashboard = () => {
  const [getRandomQuote] = useLazyQuery(GET_QUOTE, {
    fetchPolicy: 'no-cache',
    onCompleted: ({ getQuote }) => { console.log(getQuote)
  ...
  useQuery(GREETING, {
    ...
    onCompleted: ({ greeting }) => setHello(greeting)
  })    

  const [res, setRes] = useState()
  const [modus, setHello] = useState()
    ...
    <>
      ...
      {modus ? 
        ...
        <div role="status" className={styles.loading}>
          <svg aria-hidden="true" className="inline w-10 h-10 text-gray-200 animate-spin dark:text-gray-600 fill-fuchsia-800" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
